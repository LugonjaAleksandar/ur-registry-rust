...
class CryptoHDKey extends NativeObject {
  ...
  late FNGetAccountIndex fnGetAccountIndex =
      lib.lookup<NativeFunction<NativeGetAccountIndex>>("${nativePrefix}_get_account_index").asFunction<FNGetAccountIndex>();
  late NativeGetSourceFingerprint nativeGetSourceFingerprint =
      lib.lookup<NativeFunction<NativeGetSourceFingerprint>>("${nativePrefix}_get_source_fingerprint").asFunction();
  late NativeGetName nativeGetName = lib.lookup<NativeFunction<NativeGetName>>("${nativePrefix}_get_name").asFunction();
  late NativeGetPath nativeGetPath = lib.lookup<NativeFunction<NativeGetPath>>("${nativePrefix}_get_path").asFunction();
  late NativeGetChildrenPath nativeGetChildrenPath =
      lib.lookup<NativeFunction<NativeGetChildrenPath>>("${nativePrefix}_get_children_path").asFunction();
  late NativeGetDepth nativeGetDepth = lib.lookup<NativeFunction<NativeGetDepth>>("${nativePrefix}_get_depth").asFunction();
  late NativeGetChainCode nativeGetChainCode =
      lib.lookup<NativeFunction<NativeGetChainCode>>("${nativePrefix}_get_chain_code").asFunction();

  late NativeGetBip32Xpub nativeGetBip32Xpub =
      lib.lookup<NativeFunction<NativeGetBip32Xpub>>("${nativePrefix}_get_bip32_xpub").asFunction();
